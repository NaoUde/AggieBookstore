...
def displayGenreInv(genreOption, booksList):
    ...
    for book in booksList:
        ...
        print(f"\t{f"{author.getFname()} {author.getLname()} {author.getMname()}":<20}{book.getTitle():<30}{'2024':<9} {book.getQuantity():<5}{float(book.getPrice()):<5}{" "*15}")
        _Inventory += int(book.getQuantity())
        _Total += float(book.getPrice()) * int(book.getQuantity())
    print('\t' + ("=" * 33))
...
for line in file.readlines():
    line = line.split(",")
    book = Book(line[0], line[4], Author(line[1],line[3],line[2], line[5]), line[7], line[8], line[9], line[6])
    #(self, Genre, Title, Author, Price, Qty, ISBN, Year):
    if line[0] in inventory: inventory[line[0]] += [book]
    else: inventory[line[0]] = [book]
